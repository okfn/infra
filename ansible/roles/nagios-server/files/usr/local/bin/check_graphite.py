...
class Graphite(object):
    ...
    def generate_output(self, datapoints, *args, **kwargs):
        ...
        if self.has_numbers(crit_oob) and len(crit_oob) >= count:
            check_output['CRITICAL'].append('%s [crit=%f|datapoints=%s] http://graphite.okfn.org/dashboard#mail-metrics' %\
                (target, critical, ','.join(['%s' % str(x) for x in crit_oob])))
        elif self.has_numbers(warn_oob) and len(warn_oob) >= count:
            check_output['WARNING'].append('%s [warn=%f|datapoints=%s] http://graphite.okfn.org/dashboard#mail-metrics' %\
                (target, warning, ','.join(['%s' % str(x) for x in warn_oob])))
        else:
            check_output['OK'].append('%s [warn=%0.3f|crit=%f|datapoints=%s] http://graphite.okfn.org/dashboard#mail-metrics' %\
